...
class LshExecution
{
  public:
    ...
    vector<DataItem<T> *> *find_nearest_lsh(
        Metrics<T> *&metric,
        DataItem<T> *&query,
        double R,
        unsigned int limit)
        ...
        for (unsigned int i = 0; i < hash_tables_.size(); i++)
        {
            ...
            for (auto it = hash_tables_.at(i)->begin(bucket); it != hash_tables_.at(i)->end(bucket); ++it)
            {
                ...
                if (distance <= R || R == 0)
                {
                    ...
                    {
                        it->second->lsh_distance = distance;
                        result->push_back(it->second);
                    }
                    else
                    {
                        sort(result->begin(), result->end(), compareResult<T>);
                        if (result->at(result->size() - 1)->lsh_distance < distance)
                        {
                            continue;
                        }
