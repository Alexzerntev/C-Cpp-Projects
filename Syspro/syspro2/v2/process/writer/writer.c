...
void do_writers_job(int client_id, Settings *settings)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        while (getline(&line, &length, fp) != EOF)
        {
            ...
            if (i == 1)
            {
                printf("====== Sending name : %s.gpg\n", line);

                name_size = (short)(strlen(line) + 4);
                ...
                sprintf(bf2, "%s/%s", settings->input_dir, line);
                printf("Encrypting file: %s\n", bf2);
                encrypt(bf2, settings->id);
                sprintf(bf, "%s.gpg", line);

                result_code = write_string_in_chunks(bf, strlen(bf), settings->buffer_size, writer_pipe_fd);
            ...
            {
                remove_linebreak(line);
                sprintf(file_name_buffer, "%s/%s.gpg", settings->input_dir, line);
                ...
                printf("====== Writing file size that is : %s\n", writer_pipe_buffer);
                ...
                sprintf(bf3, "rm %s", file_name_buffer);
                system(bf3);
            }
        }
